...
A **client-side** bridging assistant for Minecraft **1.21.11 (Fabric)** with two bridging modes. It drives only vanilla inputs — right-clicking, and (in Crouch mode) the sneak key. Your movement and your camera stay completely in your hands.
...
|---|---|---|---|
| **User** | `.\dev.ps1 build` | `autobridge-0.3.0+1.21.11.jar` | Debug HUD **off** by default, no diagnostic logging, clean console |
| **Dev** | `.\dev.ps1 dev` | `autobridge-0.3.0+1.21.11-dev.jar` | Debug HUD **on** by default, full `DENIED:` logging |
...
.\dev.ps1 build         # user jar  -> build/libs/autobridge-0.3.0+1.21.11.jar
.\dev.ps1 dev           # dev jar   -> build/libs/autobridge-0.3.0+1.21.11-dev.jar
```

`dev.ps1` locates a JDK 21 on this machine automatically, so no global `JAVA_HOME` is needed.

Manual install: drop the **user** jar (`autobridge-0.3.0+1.21.11.jar`) into `.minecraft/mods/` alongside **Fabric API**.
